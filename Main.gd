...
func carve_path(pos1, pos2):
	...
	for x in range(pos1.x, pos2.x, x_diff):
		Map.set_cell(x, x_y.y, blankTile)
		Map.set_cell(x, x_y.y + y_diff, blankTile)  # widen the corridor
	for y in range(pos1.y, pos2.y, y_diff):
		Map.set_cell(y_x.x, y, blankTile)
		Map.set_cell(y_x.x + x_diff, y, blankTile)
